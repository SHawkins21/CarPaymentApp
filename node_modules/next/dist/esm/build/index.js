...
import { STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR, PUBLIC_DIR_MIDDLEWARE_CONFLICT, MIDDLEWARE_FILENAME, PAGES_DIR_ALIAS } from "../lib/constants";
...
import { BUILD_ID_FILE, BUILD_MANIFEST, CLIENT_STATIC_FILES_PATH, EXPORT_DETAIL, EXPORT_MARKER, FONT_MANIFEST, IMAGES_MANIFEST, PAGES_MANIFEST, PHASE_PRODUCTION_BUILD, PRERENDER_MANIFEST, FLIGHT_MANIFEST, REACT_LOADABLE_MANIFEST, ROUTES_MANIFEST, SERVER_DIRECTORY, SERVER_FILES_MANIFEST, STATIC_STATUS_PAGES, MIDDLEWARE_MANIFEST, APP_PATHS_MANIFEST, APP_PATH_ROUTES_MANIFEST, COMPILER_NAMES, APP_BUILD_MANIFEST, FLIGHT_SERVER_CSS_MANIFEST, RSC_MODULE_TYPES, FONT_LOADER_MANIFEST, CLIENT_STATIC_FILES_RUNTIME_MAIN_APP, APP_CLIENT_INTERNALS, SUBRESOURCE_INTEGRITY_MANIFEST, MIDDLEWARE_BUILD_MANIFEST, MIDDLEWARE_REACT_LOADABLE_MANIFEST } from "../shared/lib/constants";
import { getSortedRoutes, isDynamicRoute } from "../shared/lib/router/utils";
import loadConfig from "../server/config";
import { normalizePagePath } from "../shared/lib/page-path/normalize-page-path";
import { getPagePath } from "../server/require";
import * as ciEnvironment from "../telemetry/ci-info";
import { eventBuildCompleted, eventBuildOptimize, eventCliSession, eventBuildFeatureUsage, eventNextPlugins, eventTypeCheckCompleted, EVENT_BUILD_FEATURE_USAGE, eventPackageUsedInGetServerSideProps } from "../telemetry/events";
 ...
 */ function verifyTypeScriptSetup(dir, distDir, intentDirs, typeCheckPreflight, tsconfigPath, disableStaticImages, cacheDir, numWorkers, enableWorkerThreads, isAppDirEnabled) {
...
export default async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const config = await nextBuildSpan.traceChild("load-next-config").traceAsyncFn(()=>loadConfig(PHASE_PRODUCTION_BUILD, dir, conf));
                ...
                try {
                    const [[verifyResult, typeCheckEnd]] = await Promise.all([
                        nextBuildSpan.traceChild("verify-typescript-setup").traceAsyncFn(()=>verifyTypeScriptSetup(dir, config.distDir, [
                                pagesDir,
                                appDir
                            ].filter(Boolean), !ignoreTypeScriptErrors, config.typescript.tsconfigPath, config.images.disableStaticImages, cacheDir, config.experimental.cpus, config.experimental.workerThreads, isAppDirEnabled).then((resolved)=>{
                        ...
                        shouldLint && nextBuildSpan.traceChild("verify-and-lint").traceAsyncFn(async ()=>{
                            var ref;
                            await verifyAndLint(dir, eslintCacheDir, (ref = config.eslint) == null ? void 0 : ref.dirs, config.experimental.cpus, config.experimental.workerThreads, telemetry, isAppDirEnabled && !!appDir);
                        }), 
                    ]);
                    ...
                    if (!ignoreTypeScriptErrors && verifyResult) {
                        ...
                        telemetry.record(eventTypeCheckCompleted({
                            ...
                            inputFilesCount: (ref7 = verifyResult.result) == null ? void 0 : ref7.inputFilesCount,
            ...
            const pagesPaths = pagesDir ? await nextBuildSpan.traceChild("collect-pages").traceAsyncFn(()=>recursiveReadDir(pagesDir, new RegExp(`\\.(?:${config.pageExtensions.join("|")})$`))) : [];
            let appPaths;
            if (appDir) {
                appPaths = await nextBuildSpan.traceChild("collect-app-paths").traceAsyncFn(()=>recursiveReadDir(appDir, new RegExp(`^page\\.(?:${config.pageExtensions.join("|")})$`)));
            }
            const middlewareDetectionRegExp = new RegExp(`^${MIDDLEWARE_FILENAME}\\.(?:${config.pageExtensions.join("|")})$`);
            const rootDir = path.join(pagesDir || appDir, "..");
            const rootPaths = (await flatReaddir(rootDir, middlewareDetectionRegExp)).map((absoluteFile)=>absoluteFile.replace(dir, ""));
            ...
            const hasPages404 = (ref6 = mappedPages["/404"]) == null ? void 0 : ref6.startsWith(PAGES_DIR_ALIAS);
            ...
            await (async ()=>{
                ...
                await runWebpackSpan.traceAsyncFn(async ()=>{
                    ...
                    const edgeServerResult = configs[2] ? await runCompiler(configs[2], {
                        runWebpackSpan
                    }) : null;
                            ...
                            try {
                                ...
                                if (pageType === "app" && originalAppPath) {
                                    ...
                                    if (isEdgeRuntime(pageRuntime)) {
                                        ...
                                        if (!isDynamicRoute(page) && ((ref11 = workerResult.appConfig) == null ? void 0 : ref11.revalidate) !== 0) {
            ...
            if (config.experimental.preCompiledNextServer) {
                ...
                if (config.experimental.turbotrace) {
                    ...
                    if (!(binding == null ? void 0 : binding.isWasm)) {
                        var ref2;
                        nodeFileTrace = (ref2 = binding.turbo) == null ? void 0 : ref2.startTrace;
                    }
                }
                if (!nodeFileTrace) {
                    ...
                    for (let page of pageKeys.pages){
                        await includeExcludeSpan.traceChild("include-exclude", {
                            ...
                            if (excludeGlobs == null ? void 0 : excludeGlobs.length) {
                                const resolvedGlobs = excludeGlobs.map((exclude)=>path.join(dir, exclude));
                    ...
                    for (const [originalAppPath1, routes1] of appStaticPaths){
                        ...
                        if (!hasDynamicData && isDynamicRoute(originalAppPath1)) {
                            ...
                            finalDynamicRoutes[page] = {
                                ...
                                dataRouteRegex: normalizeRouteRegex(getNamedRouteRegex(dataRoute.replace(/\.rsc$/, "")).re.source.replace(/\(\?:\\\/\)\?\$$/, "\\.rsc$"))
                            };
                        }
                    }
            ...
            if (ssgPages.size > 0 || appDir) {
                ...
                await generateClientSsgManifest(prerenderManifest, {
                    distDir,
                    buildId,
                    locales: ((ref3 = config.i18n) == null ? void 0 : ref3.locales) || []
                });
            } else {
                const prerenderManifest = {
                    version: 3,
